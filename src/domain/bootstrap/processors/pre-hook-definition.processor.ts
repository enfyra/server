...
export class PreHookDefinitionProcessor extends BaseTableProcessor {
  ...
  async transformRecords(records: any[], _context?: any): Promise<any[]> {
    ...
    const transformedRecords = await mapSequentially(records, async (hook) => {
      ...
      return normalizeScriptRecord('enfyra_pre_hook', transformedHook);
    });
  ...
  async afterUpsert(
    ...
    const isMongoDB = DatabaseConfigService.instanceIsMongoDb();
    if (isMongoDB) {
      await this.syncMongoHookMethodRelation(record, 'enfyra_pre_hook');
      return;
    }
