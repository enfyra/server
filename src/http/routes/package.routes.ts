...
export function registerPackageRoutes(app: Express, container: AwilixContainer<Cradle>) {
  app.post('/package_definition', async (req: any, res: Response) => {
    const cdnLoader = req.scope?.cradle?.packageCdnLoaderService ?? container.cradle.packageCdnLoaderService;
    const queryBuilder = req.scope?.cradle?.queryBuilderService ?? container.cradle.queryBuilderService;
    const websocketGateway = req.scope?.cradle?.dynamicWebSocketGateway ?? container.cradle.dynamicWebSocketGateway;
    ...
    const packageRepo = req.routeData?.context?.$repos?.main || req.routeData?.context?.$repos?.package_definition;
    ...
    if (existingPackages.data && existingPackages.data.length > 0) {
      const { ValidationException } = await import('../../core/exceptions/custom-exceptions');
      throw new ValidationException(`Package ${body.name} (${body.type}) already installed`);
    }
    ...
    if (body.type === 'Server') {
      executeCdnLoad(cdnLoader, queryBuilder, eventEmitter, websocketGateway, savedPackageId, body.name, body.version || 'latest').catch((error) => {
    ...
    const result = await packageRepo.find({ where: { id: { _eq: savedPackageId } } });
    res.json(result);
  });
  ...
  app.patch('/package_definition/:id', async (req: any, res: Response) => {
    ...
    if (!packageRecord) {
      const { ResourceNotFoundException } = await import('../../core/exceptions/custom-exceptions');
    ...
    executeCdnUpdate(cdnLoader, queryBuilder, eventEmitter, websocketGateway, id, packageRecord.name, body.version).catch((error) => {
  ...
  app.delete('/package_definition/:id', async (req: any, res: Response) => {
    ...
    emitEvent(websocketGateway, 'uninstalled', { id, name: packageRecord.name });
    res.json(result);
  });
}
...
function emitEvent(websocketGateway: any, event: string, data: any) {
  try {
    ...
    websocketGateway.emitToNamespace(ENFYRA_ADMIN_WEBSOCKET_NAMESPACE, `${SYSTEM_EVENT_PREFIX}:${event}`, data);
  } catch (error) {
    console.warn(`Failed to emit WS event ${event}:`, error);
  }
}

async function updateStatus(queryBuilder: any, id: string | number, status: string, extra?: Record<string, any>) {
  try {
    await queryBuilder.update('package_definition', { where: [{ field: 'id', operator: '=', value: id }] }, { status, ...extra });
  } catch (error) {
    console.error(`Failed to update status to ${status} for package ${id}:`, error);
  }
}

async function executeCdnLoad(cdnLoader: any, queryBuilder: any, eventEmitter: any, websocketGateway: any, id: string | number, name: string, version: string) {
  try {
...
async function executeCdnUpdate(cdnLoader: any, queryBuilder: any, eventEmitter: any, websocketGateway: any, id: string, name: string, newVersion: string) {
  try {
