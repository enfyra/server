...
export function registerAuthRoutes(app: Express, container: AwilixContainer<Cradle>) {
  app.post('/auth/login', async (req: any, res: Response) => {
    const authService = req.scope?.cradle?.authService ?? container.cradle.authService;
