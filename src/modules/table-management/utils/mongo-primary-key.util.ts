...
export { MONGO_PRIMARY_KEY_NAME, MONGO_PRIMARY_KEY_TYPE };

export function isMongoPrimaryKeyColumn(column: {
  name?: string;
  isPrimary?: boolean;
}) {
  return (
    column.isPrimary === true && (column.name === '_id' || column.name === 'id')
  );
}
...
export function normalizeMongoPrimaryKeyColumn<
  T extends { name?: string; type?: string; isPrimary?: boolean },
>(column: T): T {
