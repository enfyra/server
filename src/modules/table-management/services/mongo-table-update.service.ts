...
export class MongoTableUpdateService extends MongoTableHandlerService {
  ...
  async updateTable(
    ...
    const out = await this.runWithSchemaLock(
      `mongo:update:${id}`,
      async () => {
        ...
        try {
          ...
          const requestedUniques =
            body.uniques ?? exists.uniques ?? oldMetadata?.uniques;
          const constraints = normalizeTableConstraints({
            uniques: requestedUniques,
            indexes: body.indexes,
            columns: body.columns || [],
            renames: this.getConstraintFieldRenames(oldMetadata, body),
            allowedFields: allowedConstraintFields ?? undefined,
          });
          const bodyUniques = constraints.uniques;
          const bodyIndexes =
            body.indexes === undefined ? undefined : constraints.indexes;
          ...
          if (
            'uniques' in body ||
            (body.columns || []).some(
              (column: any) => column.isUnique !== undefined,
            )
          ) {
            updateData.uniques = bodyUniques;
          }
          if ('indexes' in body) updateData.indexes = bodyIndexes;
          if ('isSingleRecord' in body)
            updateData.isSingleRecord = body.isSingleRecord;
          if ('validateBody' in body)
            updateData.validateBody = body.validateBody;
          ...
          if (
            ...
            this.assertNotAborted();
            stepLog(
              `STEP 11 running schemaMigrationService.updateCollection...`,
            );
            try {
          ...
          if (
            runtimeContract &&
            (renamedColumns.length > 0 || removedFields.length > 0)
          ) {
            this.assertNotAborted();
            ...
            stepLog(
              `STEP 13 applied runtime physical field changes (+${lap()}ms)`,
            );
