...
export function formatToolResultSummary(toolName: string, toolArgs: any, result: any): string {
  ...
  if (name === 'create_records') {
    ...
    if (result?.error) {
      ...
      if (result.errorCode === 'INVALID_ROUTE_DEFINITION_PAYLOAD') {
        return `[create_records] ${table} -> ERROR: Do NOT include mainTable in route_definition. mainTable is system-managed. Remove mainTable and retry.`;
      }
  ...
  if (name === 'update_records') {
    ...
    if (result?.error) {
      ...
      if (result.errorCode === 'INVALID_ROUTE_DEFINITION_PAYLOAD') {
        return `[update_records] ${table} -> ERROR: Do NOT include mainTable in route_definition. mainTable is system-managed. Remove mainTable from data and retry.`;
      }
      const message = typeof result.error === 'string' ? result.error : (result.message || result.userMessage || JSON.stringify(result.error));
      const errorMessage = truncateString(message, 500);
      const errorCode = result.errorCode ? ` (${result.errorCode})` : '';
