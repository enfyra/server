...
export class SqlSchemaMigrationService {
    ...
    try {
      ...
      if (tableMetadata.uniques?.length > 0) {
        for (const uniqueGroup of tableMetadata.uniques) {
          ...
          this.logger.log(`  Creating unique constraint on columns: ${translatedGroup.join(', ')}`);
      ...
      if (tableMetadata.indexes?.length > 0) {
        for (const indexGroup of tableMetadata.indexes) {
          ...
          this.logger.log(`  Creating index on columns: ${translatedGroup.join(', ')}`);
  ...
  private async updateMetadataFields(tableName: string, diff: any): Promise<void> {
    ...
    if (diff.metadataUpdate.uniques !== undefined) {
      ...
      this.logger.log(`  📝 Updating uniques metadata for ${tableName}`);
    }

    if (diff.metadataUpdate.indexes !== undefined) {
      updateData.indexes = JSON.stringify(diff.metadataUpdate.indexes);
      this.logger.log(`  📝 Updating indexes metadata for ${tableName}`);
    }
    ...
    if (Object.keys(updateData).length > 0) {
      try {
        await knex('table_definition')
          ...
          .update(updateData);
        this.logger.log(`  ✅ Updated metadata fields for ${tableName}: ${Object.keys(updateData).join(', ')}`);
  ...
  private async removeColumnFromMetadataIndexes(tableName: string, columnName: string): Promise<void> {
    ...
    try {
      ...
      this.logger.log(`  📝 Removed indexes referencing "${columnName}" from ${tableName} metadata`);
    } catch (error) {
  ...
  private analyzeConstraintChanges(oldMetadata: any, newMetadata: any, diff: any): void {
    ...
    for (const rename of diff.columns.rename || []) {
      ...
      this.logger.log(`  Detected column rename: ${rename.oldName} → ${rename.newName}`);
    }
    ...
    for (const [relId, newRel] of newRelMap) {
      ...
      if (oldRel && oldRel.propertyName !== newRel.propertyName) {
        ...
        if (oldFk !== newFk) {
          ...
          this.logger.log(`  Detected FK column rename from relation: ${oldFk} → ${newFk}`);
        }
    ...
    if (columnRenames.size > 0 || propertyNameRenames.size > 0) {
      ...
      if (JSON.stringify(finalUniques) !== JSON.stringify(oldUniques)) {
        this.logger.log(`  🔧 Uniques updated due to column/propertyName renames:`, { old: oldUniques, new: finalUniques });
        ...
        this.logger.log(`  Unique constraints unchanged (no column/propertyName rename impact)`);
      }

      if (JSON.stringify(finalIndexes) !== JSON.stringify(oldMetadata.indexes || [])) {
        this.logger.log(`  🔧 Indexes updated due to column/propertyName renames:`, { old: oldMetadata.indexes, new: finalIndexes });
    ...
    if (toDelete.length === 0 && toCreate.length === 0 && columnRenames.size === 0 && propertyNameRenames.size === 0) {
  ...
  private async updateMetadataIndexes(
    ...
    try {
      ...
      this.logger.log(`  📝 Updated indexes metadata for ${tableName}: added ${newIndexes.length} auto-generated indexes`);
      await this.metadataCacheService.reload();
    } catch (error) {
