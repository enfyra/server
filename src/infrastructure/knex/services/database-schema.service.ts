...
export class DatabaseSchemaService {
  ...
  private async getAllMySQLTableSchemas(knex: any): Promise<Map<string, any>> {
    ...
    const [allTables, allColumns, allIndexes, allForeignKeys] =
      await Promise.all([
        ...
        knex('INFORMATION_SCHEMA.STATISTICS')
          .select('TABLE_NAME', 'INDEX_NAME', 'COLUMN_NAME', 'NON_UNIQUE')
          .where('TABLE_SCHEMA', db)
          .where('INDEX_NAME', '!=', 'PRIMARY')
          .orderBy('INDEX_NAME', 'SEQ_IN_INDEX'),
        knex('INFORMATION_SCHEMA.KEY_COLUMN_USAGE')
          .select(
            'TABLE_NAME',
            'COLUMN_NAME',
  ...
  private async getAllPostgreSQLTableSchemas(
    knex: any,
  ): Promise<Map<string, any>> {
    const [allTables, allColumns, allPrimaryKeys, allIndexes] =
      await Promise.all([
        knex('information_schema.tables')
          .select('table_name')
    ...
    for (const tableName of tableNames) {
      ...
      const pks = new Set(
        allPrimaryKeys
          ...
          .map((p: any) => p.column_name),
      );

      const transformedColumns = cols.map((col: any) => ({
        name: col.name,
        type: this.mapPostgreSQLDataType(col.type, col),
        isPrimary: pks.has(col.name),
      ...
      for (const idx of idxs) {
        ...
        if (columnsMatch) {
          ...
          if (isUnique) uniques.push(columns);
          else regularIndexes.push(columns);
        }
      }
  ...
  private groupMySQLIndexes(indexes: any[]): {
    uniques: string[][];
    indexes: string[][];
  } {
