...
export class OAuthService {
  ...
  private async createSession(user: any, provider: OAuthProvider): Promise<any> {
    ...
    const expiredAt = new Date(Date.now() + ms((this.configService.get<string>('REFRESH_TOKEN_REMEMBER_EXP') || '7d') as ms.StringValue));
  ...
  private generateTokens(user: any, session: any): {
    ...
    const refreshToken = this.jwtService.sign(
      ...
      { expiresIn: this.configService.get<string>('REFRESH_TOKEN_REMEMBER_EXP') as ms.StringValue }
    );
