...
export { resolveSqlRelationOnDelete } from '../utils/sql-physical-schema-contract';
    ...
    try {
      ...
      if (tableMetadata.relations) {
        for (const rel of tableMetadata.relations) {
          if (isSqlForeignKeyRelation(rel)) {
      ...
      await knex.schema.createTable(tableName, (table) => {
        ...
        for (const unique of buildSqlUniqueContracts(
          tableName,
          tableMetadata,
        )) {
          table.unique(unique.physicalColumns, unique.name);
        }
        for (const index of buildSqlIndexContracts(tableName, tableMetadata)) {
      ...
      for (const rel of tableMetadata.relations || []) {
        if (rel.type === 'one-to-many') {
          ...
          const fkColumn = getSqlRelationForeignKeyColumn({
      ...
      for (const rel of tableMetadata.relations || []) {
        if (rel.type === 'many-to-many') {
          ...
          const junctionExists = await knex.schema.hasTable(junction.tableName);
          if (junctionExists) {
            continue;
          }
          const sourcePkType = await this.getPrimaryKeyType(
          ...
          try {
            ...
            this.logger.error(
              `Failed to create junction table ${junction.tableName}: ${getErrorMessage(error)}`,
            );
            throw error;
          }
        }
