...
export function buildSqlIndexContracts(
  tableName: string,
  table: Pick<TableLike, 'indexes' | 'uniques' | 'relations' | 'columns'>,
): SqlPhysicalIndexContract[] {
  const contracts: SqlPhysicalIndexContract[] = [];
  const seen = new Set<string>();
  const uniquePhysicalKeys = new Set(
    buildSqlUniqueContracts(tableName, table).map((contract) =>
      contract.physicalColumns.join('|').toLowerCase(),
    ),
  );
  const add = (
    logicalColumns: string[],
    ...
    if (uniquePhysicalKeys.has(physicalColumns.join('|').toLowerCase())) {
      return;
    }
...
export function getSqlCanonicalConstraintGroups(
  tableName: string,
  table: Pick<TableLike, 'indexes' | 'uniques' | 'relations' | 'columns'>,
): { uniques: string[][]; indexes: string[][] } {
  return {
    uniques: buildSqlUniqueContracts(tableName, table).map(
      (contract) => contract.logicalColumns,
    ),
    indexes: buildSqlIndexContracts(tableName, table).map(
      (contract) => contract.logicalColumns,
    ),
  };
}
