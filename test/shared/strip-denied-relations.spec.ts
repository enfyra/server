...
function makeRepo({
  ...
  repo.metadataCacheService = {
    lookupTableByName: jest
      .fn()
      .mockImplementation(async (name: string) => tableMetaMap[name] ?? null),
  };
...
describe('stripDeniedFields — bypass conditions', () => {
  it('returns as-is when enforceFieldPermission is false', async () => {
    ...
    const result = await strip(repo, 'main_table', ['id', 'author.name'], {
      author: {},
    });
...
describe('stripDeniedFields — relations', () => {
  it('strips denied relation from fields string', async () => {
    const repo = makeRepo({
      policiesMap: {
        'main_table:read': [makeRelDenyPolicy('main_table', 'author')],
      },
    });
    const result = await strip(
      repo,
      'main_table',
      'id,name,author.email',
      undefined,
    );
  ...
  it('strips denied relation from deep', async () => {
    ...
    const result = await strip(repo, 'main_table', ['id'], {
      author: {},
      category: {},
    });
  ...
  it('recursively strips denied nested relation from deep', async () => {
    const repo = makeRepo({
      policiesMap: {
        'user_table:read': [makeRelDenyPolicy('user_table', 'company')],
      },
    });
    const deep = {
      author: {
        fields: ['id', 'email'],
        deep: { company: { fields: ['id'] } },
      },
    };
  ...
  it('strips nested relation from deep.fields string', async () => {
    ...
    const deep = {
      author: {
        fields: 'id,email,company.name',
        deep: { company: { fields: ['name'] } },
      },
    };
    const result = await strip(repo, 'main_table', ['id', 'author'], deep);
...
describe('stripDeniedFields — columns', () => {
  it('strips denied column from explicit fields string', async () => {
    const repo = makeRepo({
      policiesMap: {
        'main_table:read': [makeColDenyPolicy('main_table', 'name')],
      },
    });
  ...
  it('never strips primary key column', async () => {
    const repo = makeRepo({
      policiesMap: {
        'main_table:read': [makeColDenyPolicy('main_table', 'id')],
      },
    });
    const result = await strip(repo, 'main_table', ['id', 'name'], undefined);
    expect(result.fields).toEqual(['id', 'name']);
  ...
  it('strips both denied column and relation in one call', async () => {
    ...
    const result = await strip(repo, 'main_table', ['id', 'name', 'author'], {
      author: {},
    });
    expect(result.fields).toEqual(['id']);
    expect(result.deep).not.toHaveProperty('author');
...
describe('stripDeniedFields — conditional rules safety', () => {
  it('does NOT strip isPublished:false column when conditional allow rule exists', async () => {
    const repo = makeRepo({
      policiesMap: {
        'main_table:read': [makeConditionalAllowPolicy('main_table', 'secret')],
      },
    });
    const result = await strip(
      repo,
  ...
  it('does NOT strip isPublished:false relation when conditional allow rule exists', async () => {
    const repo = makeRepo({
      ...
      policiesMap: {
        'main_table:read': [
          makeConditionalRelAllowPolicy('main_table', 'secretRel'),
        ],
      },
    });
    const result = await strip(repo, 'main_table', ['id', 'secretRel'], {
      secretRel: {},
    });
  ...
  it('needsPostSql bubbles up from nested deep', async () => {
    const repo = makeRepo({
      ...
      policiesMap: {
        'user_table:read': [makeConditionalAllowPolicy('user_table', 'salary')],
      },
    });
...
describe('hasConditionalRulesForField — direct', () => {
  it('returns true when conditional rule matches field', async () => {
    ...
    const result = await (repo as any).hasConditionalRulesForField(
      'main_table',
      'read',
      'column',
      'secret',
  ...
  it('returns true for conditional relation rule', async () => {
    const repo = makeRepo({
      policiesMap: {
        'main_table:read': [
          makeConditionalRelAllowPolicy('main_table', 'author'),
        ],
      },
    });
    const result = await (repo as any).hasConditionalRulesForField(
...
describe('DynamicRepository.find — stripDeniedFields integration', () => {
  function makeFullRepo({
    ...
    repo.metadataCacheService = {
      lookupTableByName: jest
        .fn()
        .mockImplementation(async (name: string) => TABLE_META[name] ?? null),
      getDirectMetadata: jest
        .fn()
        .mockReturnValue({ tables: new Map(), tablesList: [] }),
    };
    repo.fieldPermissionCacheService = {
      getPoliciesFor: jest
        .fn()
    ...
    repo.settingCacheService = {
      getMaxQueryDepth: jest.fn().mockReturnValue(5),
    };
    return repo;
  }
