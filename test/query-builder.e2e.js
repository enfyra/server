...
async function loadMetadata() {
  const tables = await db('enfyra_table').select('*');
  const columns = await db('enfyra_column').select('*');
  const relations = await db('enfyra_relation')
    .select([
      'enfyra_relation.*',
      'targetTable.name as targetTableName',
      'sourceTable.name as sourceTableName',
    ])
    .leftJoin(db('enfyra_table').as('targetTable'), 'enfyra_relation.targetTableId', 'targetTable.id')
...
async function testFilterOperators() {
  ...
  const resultEq = await executor.execute({
    tableName: 'enfyra_table',
    fields: ['id', 'name'],
    filter: { name: { _eq: 'enfyra_table' } },
    metadata,
  });

  log('Filter _eq works', resultEq.data.length === 1 && resultEq.data[0].name === 'enfyra_table');

  const resultNeq = await executor.execute({
    tableName: 'enfyra_table',
    fields: ['id', 'name'],
    filter: { name: { _neq: 'enfyra_table' } },
    limit: 5,
    metadata,
  });

  log('Filter _neq works', resultNeq.data.every(d => d.name !== 'enfyra_table'));
...
async function testLogicalOperators() {
  ...
  const resultOr = await executor.execute({
    ...
    filter: {
      _or: [
        { name: { _eq: 'enfyra_table' } },
        { name: { _eq: 'enfyra_column' } }
      ]
    },
    metadata,
  });
  ...
  const resultNot = await executor.execute({
    ...
    fields: ['id', 'name'],
    filter: {
      _not: { name: { _eq: 'enfyra_table' } }
    },
    limit: 5,
    metadata,
  });

  log('Filter _not works', resultNot.data.every(d => d.name !== 'enfyra_table'));
