...
function validateNoDuplicateInverseRelation(
  sourceTableId: number,
  ...
  let inverseRelationInfo: { propertyName: string; type: string } | null = null;

  if (rel.type === 'many-to-one' || rel.type === 'one-to-one') {
    const matching = existingRelationsOnTarget.filter(
      (r) =>
        r.sourceTableId === targetTableId &&
        r.targetTableId === sourceTableId &&
        ((rel.mappedBy && r.propertyName === rel.mappedBy) ||
          r.mappedById !== undefined),
    );
    if (matching.length > 0) {
      inverseExists = true;
      inverseRelationInfo = {
        propertyName: matching[0].propertyName,
...
describe('Inverse relation auto-creation', () => {
  ...
  describe('validateNoDuplicateInverseRelation', () => {
    it('rejects M2O when target already has inverse pointing back', () => {
      const existingOnTarget: TRelation[] = [
        makeRel({
          propertyName: 'tasks',
          type: 'one-to-many',
      ...
      expect(() =>
        validateNoDuplicateInverseRelation(
          tasksTable.id,
          tasksTable.name,
          newRel,
          usersTable.id,
      ...
      ).toThrow(/inverse relation already exists/i);
    });

    it('rejects O2M with mappedBy when target has matching owning relation', () => {
      const existingOnTarget: TRelation[] = [
        makeRel({
          propertyName: 'author',
          type: 'many-to-one',
    ...
    it('rejects M2M with mappedBy when target has matching M2M', () => {
      const existingOnTarget: TRelation[] = [
        makeRel({
          propertyName: 'tags',
          type: 'many-to-many',
    ...
    it('allows M2M without mappedBy (no duplicate check for owning side)', () => {
      const existingOnTarget: TRelation[] = [
        makeRel({
    ...
    it('allows O2M without mappedBy to skip check (but validateRelations catches this separately)', () => {
      const existingOnTarget: TRelation[] = [];
