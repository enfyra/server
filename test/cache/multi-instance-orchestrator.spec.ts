describe('CacheOrchestrator — multi-instance Redis sync', () => {
  ...
  function createInstance(id: string, redisChannel: Signal[]) {
    ...
    const stepMap: Record<string, () => Promise<void>> = {
      ...
      fieldPermission: async () => {
        reloaded.push('fieldPermission');
      },
      package: async () => {
        reloaded.push('package');
      },
      setting: async () => {
        reloaded.push('setting');
      },
      settingGraphql: async () => {
        reloaded.push('settingGraphql');
      },
    };
  ...
  function deliverSignals(
    redisChannel: Signal[],
    instances: ReturnType<typeof createInstance>[],
  ) {
    return Promise.all(
  ...
  describe('race condition: no duplicate pending/done across instances', () => {
    it('client connected to A should receive exactly 1 pending + 1 done', async () => {
      ...
      const pendingCount = A.wsEvents.filter(
        (e) => e.status === 'pending',
      ).length;
